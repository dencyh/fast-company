...
const SearchStatus = ({usersCount}) => {
  function showPeopleCount() {
    ...
    const message =
      usersCount > 0 ? `${usersCount} ${subject} ${predicate} с тобой сегодня` : "Никто с тобой не тусанет";
    return message;
  }

  return (
    <h1 className='m-2'>
      <span className={usersCount ? "badge bg-primary" : "badge bg-danger"}>{showPeopleCount()}</span>
    </h1>
  );
};
